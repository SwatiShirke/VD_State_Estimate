...
class World(object):
    ...
    def restart(self):
        ...
        self.ROS_node = CarlaROS2Publisher(self.player)
...
def game_loop(args):
    ...
    try:
        ...
        current_map = sim_world.get_map()  # Get the name of the currently loaded tow
        print(current_map)
        if args.sync:
            ...
            if not settings.synchronous_mode:
                ...
                settings.fixed_delta_seconds = T_sim
        ...
        while True:
            ...
            if controller.parse_events(client, world, clock, args.sync):
                 return
            # #world.ROS_node.publish_odometry()
            #world.ROS_node.publish_waypoints()
            world.tick(clock)
...
def get_trajectory(vehicle, N):
    """
    Retrieve the trajectory with N waypoints ahead of the current vehicle location.
    """
    # Get the map and current location
    ...
    trajectory = [(wp.transform.location.x, wp.transform.location.y, wp.transform.location.z) for wp in waypoints]
    return trajectory, current_location
...
class CarlaROS2Publisher(Node):
    ...
    def publish_waypoints(self, N=10):       
        ...
        for wp in waypoints:
            ...
            pose_stamped.pose.orientation.x = math.radians(wp.transform.rotation.yaw)
            pose_stamped.pose.orientation.y = math.radians(wp.transform.rotation.pitch)
            pose_stamped.pose.orientation.z = math.radians(wp.transform.rotation.roll)
    ...
    def get_time_spanned_waypoints(self):
        ...
        for _ in range(int(N)):
            ...
            distance = max(longitudinal_speed * T_sim, 0.1 ) # Ensure non-zero distance
            #print("dist ", distance)
